...
{
    [CustomPropertyDrawer(typeof(AIDecision))]
    public class AIDecisionCustomPropertyDrawer : PropertyDrawer
    {
        ...
        {
            AIDecision thisAction = prop.objectReferenceValue as AIDecision;
            AIDecision[] actions = (prop.serializedObject.targetObject as AIBrain).GetAttachedDecisions();
            int selected = 0;
            int i = 1;
            string[] options = new string[actions.Length + 1];
            options[0] = "None";
            foreach (AIDecision action in actions)
            {
